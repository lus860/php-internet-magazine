                                ...
                                <li class="dd-item list-group-item" data-id="{{$item->id}}" title="id">
                                    ...
                                    <div class="dd-option-handle row">
                                        ...
                                        <div class="col-1" title="{{ __('menu.status') }}">
                                            ...
                                            @endif</div>
                                        <div class="col-2"
                                             title="{{ __('menu.created_at') }}">{{$item->created_at}}</div>
                                        <div class="col-2"
                                             title="{{ __('menu.updated_at') }}">{{$item->updated_at}}</div>
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content bg-danger">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">@if (count($item->childrens) >0) You
                                                            cannot remove this substitution @else Do you really want to
                                                            delete @endif </h4>
                                                    ...
                                                    @if (count($item->childrens) == 0)
                                                        <div class="modal-footer justify-content-between">
                                                            ...
                                                            <form action="{{ route('menu_destroy', $item->id)}}"
        ...
        <div class="text-right">
            <form method="POST" action="{{ route('menu_sorting') }}">
                ...
                <button type="submit" class="btn btn-success"
                        style="margin:15px 0;">{{ __('menu.save_sorting') }}</button>
            </form>
        </div>
        <!-- /.card -->
    </section>
